...
std::unique_ptr<PersistentCacheTier> NewMyCache() {
  ...
  std::unique_ptr<PersistentCacheTier> cache(new myCache(opt));
...
class CacheTierBenchmark {
 ...
 private:
  ...
  void Prepop() {
    ...
    cache_->TEST_Flush();
    // warmup the cache
    for (uint64_t i = 0; i < 50 * 1024; ReadKey(i++)) {
    }
  }
  ...
  void ReadKey(const uint64_t val) {
    ...
    if (!FLAGS_benchmark) {
      ...
      if (memcmp(block.get(), expected_block.get(), FLAGS_iosize) != 0) {
        char *t1 = block.get(), *t2 = expected_block.get();
        printf("val1:\n");
        for (int i = 0; i < FLAGS_iosize; i++) {
          printf("%c", t1[i]);
        }
        printf("\nval2:\n");
        for (int i = 0; i < FLAGS_iosize; i++) {
  ...
  std::unique_ptr<char[]> NewBlock(const uint64_t val) {
    ...
    uint64_t t = val % 255 + 1; //避免出现 '\0'
    memset(data.get(), t, FLAGS_iosize);
    return data;
  }
